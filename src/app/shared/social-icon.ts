...
import { SocialLink } from '../data/content';

@Component({
  selector: 'px-social-icon',
  ...
  template: `
    @switch (icon()) {
      ...
      @case ('linkedin') {
        <svg viewBox="0 0 24 24" class="size-4 shrink-0 fill-current" aria-hidden="true">
          <path
...
export class SocialIcon {
  readonly icon = input.required<SocialLink['icon']>();
}
